...
interface Post {
  ...
  professor_id: string // Add professor_id to the interface
}
...
export function PostManager({ userId }: PostManagerProps) {
  ...
  const [allPosts, setAllPosts] = useRealtimeTable<Post>('scientific_outreach', [])

  // Filter posts by the current user's ID
  const posts = allPosts.filter(post => post.professor_id === userId)
  ...
  const handleDelete = async (postId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // No need to manually setPosts here
      } else {
